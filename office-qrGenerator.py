...
def transform_schedule(api_data, date_keys_to_show, time_suffix, start_hour, end_hour): 
    ...
    for entry in schedule:
        try:
            ...
            print(f"⚠️ Zamanlama verisi işlenirken hata: {e}")
    
    return ders_programi

def check_if_slot_is_current(day_name, hour_str, time_suffix):
    """
    Suffix'i dikkate alarak şu an o aralıkta mıyız kontrol eder.
    """
    try:
        now = datetime.now()
...
class RoomScheduleApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # Main Frame artık tek parça, sütunlara bölünmedi (QR kalktı)
        ...
        # Content Frame (Tablo Alanı - Artık Tüm Alan)
        self.content_frame = tk.Frame(self.main_frame, bg=self.colors["background"])
        self.content_frame.grid(row=0, column=0, sticky="nsew", padx=10)
    ...
    def master_update_loop(self):
        """Her 30 saniyede bir veri güncelle (QR yenileme kaldırıldı)."""
    ...
    def process_api_queue(self):
        try:
            while not self.api_queue.empty():
                ...
                if task_name == "room_name":
                    self.room_name = data or "Oda Yok"
                    # QR alanı kalktığı için oda ismini bir yere basmıyoruz, 
                    # ama data elimizde olsun. İstenirse title'a eklenebilir.
                    self.title(f"Oda Rezervasyon - {self.room_name}")
    ...
    def default_profile_image(self, size=(100, 100)):
        # 1. Şeffaf (RGBA) bir kare zemin oluştur
        img = Image.new('RGBA', size, (0, 0, 0, 0))
        
        # 2. Çizim aracını başlat
        draw = ImageDraw.Draw(img)
        
        # 3. İçine tam boyutta bir daire çiz (Rengi 'primary' rengin yapıyoruz)
        draw.ellipse((0, 0, size[0], size[1]), fill=self.colors["primary"])
    ...
    def build_schedule_view(self):
        """
        GÜNCELLENDİ: QR kalktığı için genişlikler tam ekrana yayıldı.
        """
        self.day_header_widgets = []
        self.schedule_cell_widgets = []
        for _ in range(5):
        ...
        self.hours = [f"{h:02}{self.time_suffix}" for h in range(self.start_hour, self.end_hour)]
        
        grid_frame = self.schedule_view_frame
        
        # GÜNCELLENDİ: Boyut Hesaplamaları (QR alanı olmadığı için genişlik artırıldı)
        saat_sutunu_genisligi = int(self.app_width * 0.06)
        # 0.72 yerine 0.98 kullanıyoruz (padding payı hariç tam ekran)
        icerik_cercevesi_genisligi = (self.app_width * 0.98) - saat_sutunu_genisligi 
        hucre_genisligi = int(icerik_cercevesi_genisligi / 5)
    ...
    def build_detail_view(self):
        ...
        detail_box.grid(row=0, column=0, sticky="new", pady=(self.app_height*0.05), padx=(self.app_width*0.1))
    ...
    def update_schedule_widgets(self):
        ...
        today = datetime.now()
        current_day_index = today.weekday() # Pazartesi=0...
        
        suffix_minute = int(self.time_suffix.replace(":", ""))
        target_hour = today.hour
        if today.minute < suffix_minute:
            target_hour -= 1
        ...
        for i in range(5):
            ...
            # Bugün ise başlığı farklı renklendir (Slayt olmadığı için index kontrolü gün ismine göre değil, weekday'e göre)
        ...
        for i in range(5): 
            ...
            for hour in self.hours:
                ...
                if status == "Boş":
                    ...
                    MAX_LEN = 20 # Genişlik arttığı için karakter limitini arttırdım
                ...
                # Sadece bugün ve şu anki saatse highlight et
                if hour == current_hour_str and i == current_day_index:
    ...
    def update_detail_widgets(self, details = False):
        ...
        for i, person in enumerate(participants):
            ...
            tk.Label(person_frame, image=img, bg=self.colors["light"]).pack(pady=(10,0))
            tk.Label(
                person_frame, 
    ...
    def check_for_current_meeting(self):
        ...
        # Eğer haftasonuysa (Cumartesi=5, Pazar=6) programda bugünü arama, çünkü program Pzt-Cum
        if current_day_index > 4:
            return

        today_date_key = self.date_keys[current_day_index]
        found_meeting = False
        today_tr = self.days_tr_turkish[current_day_index]
